...
private:
    struct Node {
        std::string fn, ln; // stands for FirstName LastName respectively
        InfoType val;       // stands for Value
        Node *prev;
        Node *next;
    };
    Node *head;
...
public:
    ...
    PeopleList(const PeopleList& rhs); // Copy constructor (deep copy)
    
    const PeopleList& operator= (const PeopleList& rhs); //assignment operator (deep copy)
